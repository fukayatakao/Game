	...
	public class BehaviorTreeEditor : EditorWindow {
		...
		private void OnGUI() {
			...
			using (new GUILayout.VerticalScope()) {
				...
				monitor_ = EditorGUILayout.ObjectField("Monitor", monitor_, typeof(BehaviorTreeMonitor), true) as BehaviorTreeMonitor;
			...
			DrawNode(root, monitor_!= null ? monitor_.func() : -1);
		...
		/// Node描画
		/// </summary>
		private void DrawNode(Node node, int cursor) {
			if(node.parent != null) {
			...
			if(node.Id == cursor) {
				basePoint = new Vector2(-node.position.x + position.size.x / scale / 2, -node.position.y + position.size.y / scale / 2);
				node.DrawFrame(basePoint);
			}
