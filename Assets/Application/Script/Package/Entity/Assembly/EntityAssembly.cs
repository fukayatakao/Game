...
namespace Project.Lib {
	...
	public abstract class EntityAssembly<T, U> : IEntityAssembly
		...
		protected CancellationToken token_;
		/// <summary>
		/// 有効にする
		/// </summary>
		public void Enable() {
		...
		//シングルトン有効な状態か
        public static bool IsValid() { return instance_ != null;}
        /// <summary>
        /// 明示的にインスタンスを作る
        /// </summary>
        public static IEntityAssembly CreateInstance(Transform root, CancellationToken token) {
			Debug.Assert(instance_ == null, "already create instance");
            instance_ = new U();
			instance_.Init(token);
