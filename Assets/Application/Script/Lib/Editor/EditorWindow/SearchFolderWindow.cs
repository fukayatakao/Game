...
namespace Project.Lib {
	public class SearchFolderWindow : EditorWindow {
		//シングルトン
		private static SearchFolderWindow instance_;
		//検索するフォルダ
		...
		/// </summary>
		[MenuItem("Editor/Sample/SearchFolderTest")]
		...
		/// 検索ウィンドウを開く
		/// </summary>
		public static SearchFolderWindow Open(string path, string ext, System.Action<string> onSelect) {
			//インスタンスが作られていなかったら生成
			if (instance_ == null) {
				instance_ = CreateInstance<SearchFolderWindow>();
			}
		...
		public void OnGUI() {
			...
			if (GUILayout.Button("フォルダを開く", GUILayout.Width(150))) {
				System.Diagnostics.Process.Start(path_);
			}
