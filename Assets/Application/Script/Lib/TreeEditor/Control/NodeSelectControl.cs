...
namespace Project.Lib.TreeEditor {
	...
	public class NodeSelectControl : IHaveControl {
		static readonly Color SELECT_COLOR = new Color(1f, 0.5f, 0.5f, 0.5f);
		//操作中か
		bool enable_;
		...
		/// ウィンドウをセットアップ
		/// </summary>
		public NodeSelectControl(TreeEditor window, UserOperation operation) {
			window_ = window;
			//自分を制御振り分け機能に登録要求
		...
		void CalcDropRect() {
			...
			if(n != null && n.Id != select_.Id) {
				drawRect = new Rect(n.innerRect_.position + window_.basePoint, n.innerRect_.size);
				window_.Repaint();
				node_ = n.Id;
			}
		}
