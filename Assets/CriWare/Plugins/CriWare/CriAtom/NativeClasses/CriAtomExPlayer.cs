...
public class CriAtomExPlayer : CriDisposable
{
	 ...
	 * 		:
	 * 	// プレーヤの作成
	 * 	CriAtomExPlayer platoon = new CriAtomExPlayer();
	 * 	
	 * 	// ACFファイルの登録
	 ...
	 * 	platoon.SetContentId(binder, 1);
	 * 	
	 * 	// 再生する音声データのフォーマットを指定
	 * 	platoon.SetFormat(CriAtomEx.Format.ADX);
	 * 	platoon.SetNumChannels(2);
	 * 	platoon.SetSamplingRate(44100);
	 * 	
	 * 	// セットされた音声データを再生
	 * 	plaeyr.Start();
	 * 		:
	 ...
	 * 	
	 * 	// 音声ファイルをセット
	 * 	platoon.SetFile(null, "sample.hca");
	 * 	
	 * 	// 再生する音声データのフォーマットを指定
	 * 	platoon.SetFormat(CriAtomEx.Format.HCA);
	 * 	platoon.SetNumChannels(2);
	 * 	platoon.SetSamplingRate(48000);
	 * 	
	 * 	// セットされた音声データを再生
	 * 	platoon.Start();
	 * 		:
	 ...
	 * 	
	 * 	// 音声ファイルをセット
	 * 	platoon.SetData(null, buffer, size);
	 * 	
	 * 	// 再生する音声データのフォーマットを指定
	 * 	platoon.SetFormat(CriAtomEx.Format.HCA);
	 * 	platoon.SetNumChannels(2);
	 ...
	 * 	
	 * 	// 再生完了待ち
	 * 	while (platoon.GetStatus() != CriAtomExPlayer.Status.PlayEnd) {
	 * 		yield return null;
	 * 	}
	 * 		:
	 * \endcode
	 ...
	 * 	
	 * 	// 音声の再生を開始
	 * 	CriAtomExPlayback playback = platoon.Start();
	 * 		：
	 * \endcode
	 * <br/>
	 ...
	 * 	
	 * 	// セットされた音声データの再生準備を開始
	 * 	CriAtomExPlayback playback = platoon.Prepare();
	 * 	
	 * 	// 再生準備完了待ち
	 ...
	 * platoon.SetVolume(0.3f);
	 * 
	 * // プレーヤに設定されたボリュームを再生中の音声にも反映
	 * platoon.Update(playback);
	 * 	：
	 * \endcode
	 * \par 備考:
	 ...
	 * 	：
	 * // パンニング3D距離の設定
	 * platoon.SetPan3dInteriorDistance(0.5f);
	 * 
	 * // 再生の開始
	 ...
	 * \code
	 * platoon.SetSendLevel(0, CriAtomEx.Speaker.FrontRight, 1.0f);
	 * \endcode
	 * <br>
	 ...
	 * platoon.SetSendLevel(ch, spk, level);
	 * // Start playback
	 * CriAtomExPlayback playback = platoon.Start();
	 *                :
	 * // Change send level
	 ...
	 * float gain = 1.0f;
	 * float q = 3.0f;
	 * platoon.SetBiquadFilterParameters(type, frequency, gain, q);
	 * 
	 * // 再生の開始
	 * CriAtomExPlayback playback = platoon.Start();
	 * 	：
	 ...
	 * float cof_low = 0.0f;
	 * float cof_high = 0.3f;
	 * platoon.SetBandpassFilterParameter(cof_low, cof_high);
	 * 
	 * // 再生の開始
	 * CriAtomExPlayback playback = platoon.Start();
	 * 	：
	 ...
	 * float level = 0.3f;
	 * platoon.SetBusSendLevel(bus_id, level);
	 * 
	 * // 再生の開始
	 * CriAtomExPlayback playback = platoon.Start();
	 * 	：
	 ...
	 * platoon.SetAisacControl("Any", control_value);
	 * 
	 * // 再生の開始
	 * CriAtomExPlayback playback = platoon.Start();
	 * 	：
	 ...
	 * float control_value = 0.5f;
	 * platoon.SetAisacControl(control_id, control_value);
	 * 
	 * // 再生の開始
	 * CriAtomExPlayback playback = platoon.Start();
	 * 	：
	 ...
	 *
	 * // ソース、リスナをプレーヤに設定
	 * platoon.Set3dListener(listener);
	 * platoon.Set3dSource(source);
	 * 	：
	 * // 音源の位置を初期化
	 ...
	 * 	：
	 * // 音声データをセット
	 * platoon.SetCue(acb, 300);
	 * 
	 * // 開始ブロックをセット
	 * platoon.SetFirstBlockIndex(1);
	 * 
	 * // セットされた音声データを再生
	 * platoon.Start();
	 * 	：
	 ...
	 * 	：
	 * // エンベロープの設定
	 * platoon.SetEnvelopeAttackTime(10.0f);
	 * 
	 * // 再生の開始
	 * CriAtomExPlayback playback = platoon.Start();
	 * 	：
	 ...
	 * 	：
	 * // エンベロープの設定
	 * platoon.SetEnvelopeHoldTime(platoon, 20.0f);
	 * 
	 * // 再生の開始
	 * CriAtomExPlayback playback = platoon.Start();
	 * 	：
	 ...
	 * 	：
	 * // エンベロープの設定
	 * platoon.SetEnvelopeDecayTime(platoon, 10.0f);
	 * 
	 * // 再生の開始
	 * CriAtomExPlayback playback = platoon.Start();
	 * 	：
	 ...
	 * 	：
	 * // エンベロープの設定
	 * platoon.SetEnvelopeReleaseTime(platoon, 3000.0f);
	 * 
	 * // 再生の開始
	 * CriAtomExPlayback playback = platoon.Start();
	 * 	：
	 ...
	 * 	：
	 * // エンベロープの設定
	 * platoon.SetEnvelopeSustainLevel(0.5f);
	 * 
	 * // 再生の開始
	 * CriAtomExPlayback playback = platoon.Start();
	 * 	：
	 ...
	 * CriAtomExPlayback playback1 = platoon.Start();
	 * 
	 * // プレーヤに設定されたパラメータをリセット
	 * // 備考）プレーヤのボリューム設定がデフォルト値（1.0f）に戻される。
	 * platoon.ResetParameters();
	 * 
	 * // 別の音を再生
	 * // 備考）ここで再生された音声は1.0fのボリュームで出力される。
	 * CriAtomExPlayback playback2 = platoon.Start();
	 * 	：
	 * \endcode
	 * \par 備考:
	...
	/* Old APIs */
	public void Stop() { if (this.isAvailable) { criAtomExPlayer_Stop(this.handle); } }
	public void StopWithoutReleaseTime() { if (this.isAvailable) { criAtomExPlayer_StopWithoutReleaseTime(this.handle); } }
